...
created: 2026-04-22
updated: 2026-05-01
sources: [claude-code-2h-mastery, harness-engineering-era, jimcoding-claude-rules, jay-choi-9-tips, matt-pocock-software-fundamentals, claude-code-agent-teams-opus46, work-prep-2026-05-01]
...
> [!note] 실무 적용 패턴 — 직접 구축 우선
> [[sources/work-prep-2026-05-01]] 본 위키 사용자 사례: *"컨텍스트 최적화를 위해서 무거운 mcp 사용 자제. 쉽게 구축 가능한 mcp 라면 직접 생성 혹은 script 파일로 대체."* 범용 MCP 의 *전부 상시 로드* 비용을 피하기 위해 **필요한 엔드포인트만 골라 자체 MCP 또는 그냥 shell script** 로 대체하는 운영 원칙. 도구 설명 토큰을 1/N 로 압축.
...
## 7) Ubiquitous Language — 어휘 정렬 축

세컨드 브레인이 *지식·결정·패턴* 의 누적이라면, **공유 어휘** 는 별도의 직교 축. [[entities/matt-pocock]] 가 [[sources/matt-pocock-software-fundamentals]] 에서 DDD 의 ubiquitous language 를 AI 어휘 정렬용으로 재해석:

- 코드베이스에서 등장하는 도메인 용어를 마크다운 표로 정리.
- 사용자도, AI 도 **항상 열어 두고 참조**.
- 효과: AI 의 사고 흔적(thinking traces) 자체가 덜 verbose 해진다 + 계획↔구현 정렬도 향상.

자세히 → [[concepts/ubiquitous-language]].

| 도구 | 공유 대상 | 산출물 |
|---|---|---|
| **세컨드 브레인** | 결정·패턴·노트 | 자유 형식 |
| **Ubiquitous Language** | 어휘 (도메인 용어) | 마크다운 표 |
| **`.claude/rules/*.md`** | 규칙 | frontmatter glob |

## 9) Agent Team 으로 컨텍스트 흘릴 때 — 사전 패키징 의무

[[concepts/agent-teams|Agent Team]] 모드는 main session 의 대화 thread 를 **자동 전달하지 않는다**. default 모드에서 30분 토론한 결정·웹서치·파일 탐색 메모는 팀원에게 *증발*. 이는 sub-agent 의 격리보다 한 단계 강한 격리.

**대응 — 사전 MD 패키징**:

```
1. default 에서 합의 도달 → docs/research-brief.md 에 컨텍스트 패키징
2. team 생성 prompt 에 "read docs/research-brief.md first" 명시
3. team leader 가 각 agent prompt 에 동일 지시 자동 포함
```

세션 간 지속에는 **shared memory MD** — 모든 agent 가 자신의 try/fail 을 한 파일에 append. 다음 세션의 새 agent 가 이 파일 먼저 읽고 시작한다.

본 항목은 [[concepts/claude-md|CLAUDE.md]] 의 *항상 로드되는 규칙* 과 다른 결: **task 단발 brief**. 영구 규칙은 CLAUDE.md, 이번 task 한정 컨텍스트는 별도 brief MD.

## 10) Mermaid 다이어그램 아키텍처
...
- [[concepts/ubiquitous-language]] — 어휘 정렬 축
...
- [[concepts/agent-teams]] — 더 강한 세션 격리 (사전 MD 조립 의무)
...
- [[sources/matt-pocock-software-fundamentals]] — ubiquitous language 어휘 정렬 축
- [[sources/claude-code-agent-teams-opus46]] — agent-team 격리에 따른 사전 MD 패키징 의무
- [[sources/work-prep-2026-05-01]] — MCP 다이어트 + script 대체 본 위키 사용자 사례
