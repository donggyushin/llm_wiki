...
created: 2026-04-22
updated: 2026-04-28
sources: [harness-engineering-era, jay-choi-9-tips, matt-pocock-software-fundamentals]
...
## 코드 레벨 하네스 — Deep Modules

환경/시스템 층에서의 하네스가 *프로젝트 안의 울타리* 라면, **코드 내부** 에도 같은 사상이 적용된다. [[entities/john-ousterhout]] 의 [[concepts/deep-modules]] 는 *AI 가 어길 수 없는 인터페이스 경계* 다:

- 사람이 인터페이스를 직접 설계 — 잘못된 인터페이스는 AI 가 망친다.
- 구현은 AI 에게 *gray box* 로 위임. 외부 테스트만 통과하면 안쪽은 안 들여다본다.
- 얕은 모듈만 가득한 코드베이스는 *코드 레벨 하네스 부재* — AI 가 의존성을 못 따라잡고 길을 잃는다.

→ [[sources/matt-pocock-software-fundamentals]] 의 처방 4·5번. 같은 사상의 두 층:

| 층 | 어길 수 없게 만드는 도구 |
|---|---|
| 환경 | 린터, architecture test, file system 권한, pre-commit hook (위 4기둥) |
| 코드 | 단순 인터페이스 + 깊은 구현 (deep module) |
...
- [[concepts/deep-modules]] — 코드 레벨 하네스
- [[concepts/code-is-not-cheap]] — 같은 결의 테제 (펀더멘털 강조)
- [[concepts/specs-to-code-antipattern]] — 하네스 부재 시 가속되는 안티패턴
...
- [[sources/matt-pocock-software-fundamentals]] — 코드 레벨 하네스 (deep modules)
